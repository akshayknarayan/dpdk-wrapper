use crate::{bindings::*, mem::closest_2mb_page, wrapper};
...
impl MempoolInfo {
    ...
    pub unsafe fn new(handle: *mut rte_mempool) -> Result<Self> {
        ...
        let hugepage_size = (*(*handle).mz).hugepage_sz as usize;
        Ok(MempoolInfo {
            handle,
            hugepage_size,
            start,
    ...
    fn header_size(&self) -> usize {
        // SAFETY: self initialized correctly
    ...
    /// Safety:
    /// `buf` is from an `rte_mbuf`.
    pub unsafe fn get_mbuf_ptr(&self, buf: &[u8]) -> Result<(*mut rte_mbuf, u16)> {
        ...
        let data_off = (ptr_int - (*original_mbuf_ptr).buf_addr as usize) as u16;
...
impl Drop for MempoolInfo {
    fn drop(&mut self) {
        // SAFETY: self must have been initialized correctly.
        unsafe { rte_mempool_free(self.handle) };
    }
}
...
impl MempoolAllocator {
    /// Safety: mempool is valid.
    pub unsafe fn add_mempool(&mut self, mempool: *mut rte_mempool, obj_size: usize) -> Result<()> {
    ...
    pub fn allocate(&self, alloc_size: usize, _alignment: usize) -> Result<*mut rte_mbuf> {
        ...
        let min_size: usize = min_size.unwrap();

        let mempools = self.mempools.get(&(min_size)).unwrap();
        for mempool in mempools.iter() {
            let mbuf = unsafe { rte_pktmbuf_alloc(mempool.get_mempool()) };
            if !mbuf.is_null() {
                return Ok(mbuf);
            }
        }
        ...
        for mempools in self.mempools.values() {
            for mempool in mempools.iter() {
                if mempool.is_within_bounds(buf) {
                    let res = unsafe { mempool.get_mbuf_ptr(buf) }?;
                    return Ok(Some(res));
                }
            }
        }
...
///
/// Safety:
/// `mbuf_pool` must be valid.
unsafe fn get_mempool_memzone_area(
    ...
    unsafe extern "C" fn memzone_callback(
        mp: *mut rte_mempool,
        opaque: *mut ::std::os::raw::c_void,
        memhdr: *mut rte_mempool_memhdr,
        idx: ::std::os::raw::c_uint,
    ) {
        let mr = &mut *(opaque as *mut Vec<(usize, usize, usize, usize)>);
        let (addr, len) = ((*memhdr).addr, (*memhdr).len);
        let hugepage_size = (*(*mp).mz).hugepage_sz as usize;
    ...
    let pagesize = (*(*mbuf_pool).mz).hugepage_sz as usize;
    ...
    tracing::debug!(
        ...
        mempool_size = (*mbuf_pool).size as usize,
        mempool_hdr_size = (*mbuf_pool).header_size as usize,
        mempool_trailer_size = (*mbuf_pool).trailer_size as usize,
        priv_size = (*mbuf_pool).private_data_size as usize,
        elt_size = (*mbuf_pool).elt_size as usize,
        "Calculated and mempool measurements"
    );

    let start = mbuf_addrs[0];
    let mut end = mbuf_addrs[(*mbuf_pool).populated_size as usize - 1];
    end += (*mbuf_pool).size as usize;
    let size = end - start;
    ...
    tracing::debug!(
        ...
        cur_data_off = (*(start as *mut rte_mbuf)).data_off as usize,
        offset_of_buffer_from_start = (*(start as *mut rte_mbuf)).buf_addr as usize - start,
        "mbuf offsets"
    );
